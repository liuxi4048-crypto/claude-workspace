...
f16 版(バッファが小さく読込に成功しやすい)を既定にし、出力が化ける端末向けに f32 版も用意しています。
...
> - **応答が「!!!!」や `<pad>` などの特殊トークン・意味不明な単語に化ける** → 同じモデルの「…安定版(f32)」を選ぶ(モバイルGPUの f16 数値不安定対策)。アプリが自動検知して警告を表示します。
>
> ※ モバイルChromeのWebGPUは、システムRAMが大きくても GPU 側のバッファ上限やデバイス安定性の制約があり、7B級以上はほぼ読み込めません。実用上限は概ね 3〜4B です。また f16/f32 のどちらでも安定しない端末では、アプリを一度完全に閉じてから開き直す(GPUコンテキストのリセット)ことも有効な場合があります。
