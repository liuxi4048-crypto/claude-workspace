...
| モデル | DL目安 | VRAM | 用途 |
|--------|--------|------|------|
| **Qwen2.5 3B**(既定) | 約2GB | 約2.9GB | 実機で安定して動く中で最も高品質。日本語・要約・推論に強い |
| Gemma 2 2B 日本語版 | 約1.4GB | 約2.5GB | 日本語チューニング済み・最も安定 |
| Qwen2.5 1.5B | 約1GB | 約1.9GB | 超軽量・高速 |
| Qwen2.5 7B | 約4.5GB | 約5.9GB | 最高品質だが**モバイルでは読込失敗しやすい**(GPU上限超過) |
| Qwen3.5 9B | 約5.5GB | 約7.5GB | 実験・最大(スマホではほぼ不可・ハイエンドPC向け) |

> **⚠ スマホのGPUメモリ制約について**: モバイルChromeのWebGPUは、システムRAMが大きくても
> GPUバッファの確保量に上限があり、**7B級以上は多くのスマホで読込に失敗**します
> (`Buffer was unmapped...` 等)。スマホでの実用上限は概ね **3〜4B** です。
>
> **読込失敗 / 応答が文字化け(!!!!)する場合**は、⚙️設定からより軽いモデル
> (特に Gemma 2 2B 日本語版)に切り替えてください。
